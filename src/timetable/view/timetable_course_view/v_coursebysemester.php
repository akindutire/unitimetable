...
$AllSemesterCourse = $data[3];

$baselink = $data[4];

$alldept = $data[5];

$semesterIndex = $data[6];
...
<article class="w3-col l12 m12 s12 w3-padding" style="margin-top: 6% !important; padding-left: 0px !important;">
	...
	<section class="w3-col l10 m9 s12">
		...
		<div class="w3-col l12 m12 s12 w3-padding" style="display: flex; justify-content: center;">
            ...
            <?php
                ...
                if(count($AllSemesterCourse) == 0)
                    goto nodata;


                    echo "<p class='w3-large w3-col l12 m12 s12'>{$semesterIndex} Semester</p>";



                        foreach ($AllSemesterCourse as  $department_course_details) {


                            echo "<table class='w3-table w3-col l12 m12 s12 w3-small'>
                                    <thead class='w3-light-gray'>
                                        <tr>
                                            <th>#</th><th>Title</th><th>Code</th><th>Unit</th> <th>Level</th> <th>Hours Required</th> <th>Department</th> <th>Straight Hours</th> <th>Class</th>  <th>Semester</th> 
                                        </tr>
                                    </thaed>
                                    <tbody>
                                ";

                            $nm = ucwords($department_course_details['name']);
                            echo "<p class='w3-xxlarge w3-center w3-col l12 m12 s12'>{$nm}</p>";
                            foreach ($department_course_details as $code => $course_details){
                                ...
                                echo "<tr class='w3-border-bottom'>
                                                    ...
                                                    <td id='title'>{$course_details[0]}</td> 
                                                    <td>{$code}</td> 
                                                    <td>{$course_details[1]}</td> 
                                                    <td>{$course_details[7]}</td>
                                                    <td id='hrsReq'>{$course_details[2]}</td> 
                                                    <td>{$course_details[5]}</td> 
                                                    <td id='strHrs'>{$course_details[3]}</td>  
                                                    <td id='noClass'>{$course_details[4]}</td>
                                                    <td>{$course_details[6]}</td>
                                                    ";
                        ...
                        nodata:echo "<p class='w3-large'>No Course found</p>";
