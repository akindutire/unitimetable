...
include_once("includes.php");
	...
	if(!empty($data->venue)){
		...
		$array = $sanitize->cleanData([$data->venue, $data->to_mark_as]);
		
		$dmgt = new dmgt;

		$feedback = $dmgt->markasmultisightvenue(intval($array[0]), $array[1] );
		
		if ($feedback == true)
			echo json_encode(["msg"=>$dmgt->msg,"success"=>1]);
		else
			echo json_encode(["msg"=>"Couldn't mark Venue, retry","success"=>0]);
		
	}else{

		echo json_encode(["msg"=>"Venue must not be empty","success"=>0]);
	}
